function replaceShared(data) {
  ...
  replaceResult(
    data.runnerInfo.date,
    "runner-date",
    new Date(data.runnerInfo.date).toLocaleString(),
  );
  replaceResult(
    data.stashInfo.version.hash,
    "runner-hash",
    ` (${data.stashInfo.version.hash})`,
  );
  // set url
  ...
  setUrl(
    "scraper-hash",
    `https://github.com/stashapp/CommunityScrapers/commit/${scraperVersion}`,
    ` (${scraperVersion})`,
  );
  ...
  if (data?.logs?.[Symbol.iterator]) {
    ...
    if (data.logs.length === 30) {
      ...
      logMessage.textContent =
        "Log limit reached (30). Some logs may have been cut off.";
      logRow.appendChild(logMessage);
      logContainer.appendChild(logRow);
    }
  }
